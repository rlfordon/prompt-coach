...
import { Platform, FocusArea, ChatMessage, CoachRequest, ModelOption, ModelType } from '../types';
import { getCoachingFeedback } from '../services/geminiService';

interface CoachPanelProps {
  chatHistory: ChatMessage[];
  workspaceModel: ModelOption;
  isPasteMode: boolean;
...
const CoachPanel: React.FC<CoachPanelProps> = ({ 
  ...
  // Auto-set defaults based on workspace selection, ONLY if not in paste mode
  useEffect(() => {
    if (isPasteMode) return;

    // Default to Gemini when syncing with workspace
    setPlatform(Platform.GEMINI);
    
    // Default model type based on workspace model
    if (workspaceModel === ModelOption.PRO_THINKING) {
      setModelType(ModelType.REASONING);
    } else {
      setModelType(ModelType.INSTANT);
    }
  }, [workspaceModel, isPasteMode]);

  // Check if model type options should be shown
  ...
  const handleReviewConversation = async () => {
    ...
    const response = await getCoachingFeedback(request);
  ...
  return (
    <div className="flex flex-col h-full bg-white">
      ...
      <div className="p-6 border-b border-gray-200 bg-gray-50 space-y-4">
        ...
        {/* Only show Platform and Model Type selection if in Paste Mode and no feedback/loading */}
        {isPasteMode && !feedback && !isLoading && (
          <>
            ...
            {/* Model Type Selection (Conditional) */}
                ...
                <div className="space-y-2">
                  <div className="flex items-center">
                    <input
                      ...
                      name="modelType"
                      type="radio"
                      checked={modelType === ModelType.INSTANT}
                      onChange={() => setModelType(ModelType.INSTANT)}
                      className="focus:ring-scarlet h-4 w-4 text-scarlet border-gray-300"
                    />
                    <label htmlFor="instant" className="ml-3 block text-sm text-gray-700">
                      Instant/Fast (e.g., Gemini Flash, GPT-4o, Claude Haiku/Sonnet)
                    </label>
                  </div>
                  <div className="flex items-center">
                    <input
                      ...
                      name="modelType"
                      type="radio"
                      checked={modelType === ModelType.REASONING}
                      onChange={() => setModelType(ModelType.REASONING)}
                      className="focus:ring-scarlet h-4 w-4 text-scarlet border-gray-300"
                    />
                    <label htmlFor="reasoning" className="ml-3 block text-sm text-gray-700">
                      Reasoning/Thinking (e.g., Gemini Pro Thinking, o1/o3, Claude Opus)
                    </label>
                  </div>
                  <div className="flex items-center">
                    <input
                      ...
                      name="modelType"
                      type="radio"
                      checked={modelType === ModelType.NOT_SURE}
                      onChange={() => setModelType(ModelType.NOT_SURE)}
                      className="focus:ring-scarlet h-4 w-4 text-scarlet border-gray-300"
                    />
                    <label htmlFor="not_sure" className="ml-3 block text-sm text-gray-700">
                      Not sure
                    </label>
                  </div>
                </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 pt-2">
          ...
          >
            {isLoading 
              ? 'Analyzing...' 
              : isPasteMode ? 'Review Pasted Conversation' : 'Review My Conversation'
            }
          </button>
